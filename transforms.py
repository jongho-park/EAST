...
def transform_by_matrix(matrix, image=None, oh=None, ow=None, word_bboxes=[],
    ...
    Args:
        ...
        by_word_char_bboxes (List[List[ndarray]]): Lists of (4, 2) shaped ndarrays.
    ...
    if by_word_char_bboxes is None:
        ...
        points = np.concatenate([np.rehsape(bboxes, (-1, 2)) for bboxes in by_word_char_bboxes])  # (N, 2)
        points = cv2.perspectiveTransform(
...
def transform_image_and_bboxes_by_matrix(matrix, oh, ow, image=None, word_bboxes=[],
                                         by_word_char_bboxes=[], masks=[], inverse=False):
    warnings.warn('`transform_image_and_bboxes_by_matrix` is deprecated, use `transform_by_matrix` \
                  instead')
    return transform_by_matrix(matrix, image=image, oh=oh, ow=ow, word_bboxes=word_bboxes,
...
class GeoTransformation:
    ...
    def crop_rotate_resize(self, image, word_bboxes=[], by_word_char_bboxes=[], masks=[]):
        ...
        if not self.cond_random_translate:
            ...
            patch_quad = self._get_located_patch_quad(ih, iw, patch_quad_rel, bboxes=word_bboxes)
        ...
        return dict(found_randomly=found_randomly, matrix=M, **transformed)
...
class ComposedTransformation:
    def __init__(
        ...
        rotate_anchors=None, rotate_range=None,
        crop_aspect_ratio=None, crop_size=1.0, crop_size_by='longest', hflip=False, vflip=False,
        cond_random_translate=False, min_image_overlap=0.5, min_bbox_overlap=0.99, min_bbox_count=0,
        resize_to=None, keep_aspect_ratio=False, resize_based_on='longest', max_random_trials=100,
        ...
        self.geo_transform_fn = GeoTransformation(
            ...
            hflip=hflip, vflip=vflip, cond_random_translate=cond_random_translate,
            min_image_overlap=min_image_overlap, min_bbox_overlap=min_bbox_overlap,
            min_bbox_count=min_bbox_count, resize_to=resize_to, keep_aspect_ratio=keep_aspect_ratio,
...
def concat_quads(word_bboxes=None, by_word_char_bboxes=None):
    """
    Returns:
        num_word_bboxes (int)
        num_char_bboxes (int)
        merged_bboxes (List[ndarray]): List of (N, 2) shaped (polygon) ndarrays.
    """
    if word_bboxes is None:
...
def transform_image_and_bboxes(transform_fn, image, word_bboxes=[], by_word_char_bboxes=[],
                               masks=[]):
    if transform_fn is None:
        return dict(image=image, matrix=None, word_bboxes=word_bboxes,
                    by_word_char_bboxes=by_word_char_bboxes, masks=masks)

    return transform_fn(image=image, word_bboxes=word_bboxes,
                        by_word_char_bboxes=by_word_char_bboxes, masks=masks)
